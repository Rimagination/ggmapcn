...
You can install the development version of ggmapcn from [GitHub](https://github.com/) with `devtools`:
...
```{r example4, fig.alt='Map with Boundary'}
ggplot() +
  geom_mapcn(fill = NA) +
  geom_boundary_cn(
...
```{r example6, fig.alt='Map of China'}
ggplot() +
  geom_buffer_cn(mainland_dist = 40000) +
  geom_buffer_cn(mainland_dist = 20000, fill = "#BBB3D8") +
